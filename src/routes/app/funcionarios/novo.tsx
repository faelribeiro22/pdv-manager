...
const inviteEmployeeFn = createServerFn({ method: 'POST' })
  .validator((d: { establishmentId: string; name: string; email: string; role: string }) => d)
  .handler(async ({ data }) => {
    ...
    return (
      <div className="p-6 max-w-md mx-auto">
        <div className="bg-white rounded-xl border border-gray-200 p-6 text-center">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-1">Funcionário Criado!</h2>
          <p className="text-gray-500 text-sm mb-5">Compartilhe as credenciais abaixo com <strong>{credentials.name}</strong></p>
          <div className="bg-gray-50 rounded-xl p-4 text-left mb-4 font-mono text-sm">
            <p className="text-gray-600">E-mail: <span className="text-gray-900 font-semibold">{credentials.email}</span></p>
            <p className="text-gray-600 mt-1">Senha: <span className="text-gray-900 font-semibold">{credentials.password}</span></p>
          </div>
          <button onClick={copyCredentials} className="w-full flex items-center justify-center gap-2 border border-gray-300 text-gray-700 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-50 mb-3">
  ...
  return (
    <div className="p-6 max-w-md mx-auto">
      <div className="flex items-center gap-4 mb-6">
        <button onClick={() => navigate({ to: '/app/funcionarios' })} className="text-gray-500 hover:text-gray-800">
          <ArrowLeft size={22} />
        </button>
        <h1 className="text-xl font-bold text-gray-900">Convidar Funcionário</h1>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <p className="text-xs text-gray-500">Uma senha aleatória será gerada e exibida após a criação.</p>
