          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {products.map((p) => {
                ...
                return (
                  <tr key={p.id} className={`hover:bg-gray-50 ${lowStock ? 'bg-amber-50/50' : ''}`}>
                    ...
                    <td className="px-4 py-3 text-right font-medium text-gray-800">{fmt(p.price)}</td>
                    <td className="px-4 py-3 text-right">
                      <span className={`font-medium ${outOfStock ? 'text-red-600' : lowStock ? 'text-amber-600' : 'text-gray-800'}`}>
                        {p.stock_qty} {p.unit}
